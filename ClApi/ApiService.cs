...
namespace ClApi
{
    public class ApiService : IApi
    {
        ...
        public ApiService(HttpClient client, string baseUri)
        {
            ...
            uri = baseUri ?? throw new ArgumentNullException(nameof(baseUri));
        }
        ...
        public Task<CityList> GetAllCitiesAsync() =>
            client.GetFromJsonAsync<CityList>($"{uri}/api/Sellect/CitySelector/CitySelector");
        ...
        //// שליפה של כל הילדים
        //public async Task<List<ChildOfParent>> GetAllChildOfParentsAsync()
        //{
        //    return await client.GetFromJsonAsync<List<ChildOfParent>>(
        //        $"{uri}/api/Sellect/SelectAllChildOfParent");
        //}

        //// שליפה לפי הורה
        //public async Task<List<ChildOfParent>> GetChildrenByParentIdAsync(int parentId)
        //{
        //    var result = await GetAllChildOfParentsAsync();

        //    if (result == null)
        //        return new List<ChildOfParent>();

        //    return result
        //        .Where(c => c.IdParent != null && c.IdParent.Id == parentId)
        //        .ToList();
        //}
        ...
        // ================= ChildOfParent =================
        public Task<ChildOfParentList> GetAllChildOfParentsAsync()
            => client.GetFromJsonAsync<ChildOfParentList>( $"{uri}/api/Sellect/SelectAllChildOfParent");

        public async Task<int> InsertChildOfParentAsync(ChildOfParent child) 
            => (await client.PostAsJsonAsync( $"{uri}/api/Sellect/InsertAChildOfParent", child)).IsSuccessStatusCode ? 1 : 0;

        public async Task<int> UpdateChildOfParentAsync(ChildOfParent child) 
            => (await client.PutAsJsonAsync( $"{uri}/api/Sellect/UpdateAChildOfParent", child)).IsSuccessStatusCode ? 1 : 0;

        public async Task<int> DeleteChildOfParentAsync(int id) 
            => (await client.DeleteAsync( $"{uri}/api/Sellect/DeleteAChildOfParent/{id}")).IsSuccessStatusCode ? 1 : 0;
